...
# Step 4: Prepare the data for MLlib by assembling power and toughness (pwtg) into a vector
assembler = VectorAssembler(
    inputCols=["power", "toughness"],
    outputCol="pwtg",
    handleInvalid="skip"  # Skip rows with null values
)
assembled_df = assembler.transform(mtg_df).select("pwtg", "color_identity")

# Step 5: Split the data into training and testing sets
train_data, test_data = assembled_df.randomSplit([0.7, 0.3])

# Step 6: Initialize and train a Linear Regression model
lr = LinearRegression(labelCol="color_identity")
...
def write_to_hbase_partition(partition):
    ...
    connection.open()
    table = connection.table('mtg_table')  # Update table name
